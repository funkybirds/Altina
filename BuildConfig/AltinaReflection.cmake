...
function(ae_add_reflection_codegen)
    ...
    # The reflection scanner consumes a compilation database. Some generators (notably Visual Studio/MSBuild)
    # do not reliably support compile_commands.json, so fall back to a stub implementation only when a
    # compilation database is not expected to be generated.
    set(ae_refl_compdb_expected FALSE)
    if (CMAKE_EXPORT_COMPILE_COMMANDS)
        if (CMAKE_GENERATOR MATCHES "Ninja" OR CMAKE_GENERATOR MATCHES "Unix Makefiles" OR CMAKE_GENERATOR MATCHES "MinGW Makefiles")
            set(ae_refl_compdb_expected TRUE)
        endif()
    endif()

    # If a previous configure fell back to a stub, remove it so the build can generate real reflection.
    if (ae_refl_compdb_expected AND EXISTS "${gen_cpp}")
        file(READ "${gen_cpp}" existing_gen)
        if (existing_gen MATCHES "Auto-generated by CMake \\(compile_commands.json missing\\)")
            file(REMOVE "${gen_cpp}")
        endif()
    endif()

    if (NOT ae_refl_compdb_expected AND NOT EXISTS "${compile_commands_root}/compile_commands.json")
        message(WARNING
            "Reflection codegen fallback for ${AE_REFL_MODULE_NAME}: "
            "missing ${compile_commands_root}/compile_commands.json. "
            "Generating an empty reflection unit instead.")
        ...
        if (AE_REFL_FORBID_ANNOTATIONS)
            ...
            set(stub_content
                "// Auto-generated by CMake (compile_commands.json missing). Do not edit.\n"
                "// Module: ${AE_REFL_MODULE_NAME}\n"
