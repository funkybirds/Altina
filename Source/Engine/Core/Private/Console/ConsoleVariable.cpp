...
const FString& FConsoleVariable::GetName() const noexcept
{
    return mName;
}
...
AE_CORE_API FConsoleVariable* FConsoleVariable::Register(const FString& name, const FString& defaultValue) noexcept
{
...
AE_CORE_API void FConsoleVariable::ForEach(TFunction<void(const FConsoleVariable&)> fn) noexcept
{
    FScopedLock lock(gRegistryMutex);
...
FConsoleVariable::EType FConsoleVariable::GuessType(const FString& v) noexcept
{
    ...
    if (s.Length() == 4 && s[0] == 't' && s[1] == 'r' && s[2] == 'u' && s[3] == 'e')
        return EType::Bool;
    if (s.Length() == 5 && s[0] == 'f' && s[1] == 'a' && s[2] == 'l' && s[3] == 's' && s[4] == 'e')
        return EType::Bool;
...
bool FConsoleVariable::FStringEqual::operator()(const FString& a, const FString& b) const noexcept
{
