...
void FWorkerPool::SubmitDelayed(AltinaEngine::Core::Container::TFunction<void()> Job, std::chrono::milliseconds Delay)
{
    FJobEntry e;
    e.Task = std::move(Job);
    e.Priority = 0;
...
void FWorkerPool::WorkerMain()
{
    while (bRunning.load() || !JobQueue.IsEmpty()) {
        ...
        if (!batch.empty()) {
            ...
            std::sort(batch.begin(), batch.end(), [](const FJobEntry &a, const FJobEntry &b){ return a.Priority > b.Priority; });
