...
FMutex::FMutex() noexcept {
    Impl = PlatformCreateCriticalSection();
}
...
void FMutex::Lock() noexcept {
    PlatformEnterCriticalSection(Impl);
}

bool FMutex::TryLock() noexcept {
    return PlatformTryEnterCriticalSection(Impl) != 0;
}

void FMutex::Unlock() noexcept {
    PlatformLeaveCriticalSection(Impl);
}

void* FMutex::GetNative() const noexcept {
    return Impl;
}

FScopedLock::FScopedLock(FMutex& InMutex) noexcept
    : Mutex(InMutex) {
    Mutex.Lock();
}

FScopedLock::~FScopedLock() noexcept {
    Mutex.Unlock();
}
