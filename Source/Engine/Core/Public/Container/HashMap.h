...
namespace AltinaEngine::Core::Container {
    ...
    public:
        ...
        explicit THashMap(size_type bucket_count, const Hash& hash = Hash(),
        ...
        THashMap(std::initializer_list<value_type> init, size_type bucket_count = 0,
            const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual(),
            const Allocator& alloc = Allocator())
            : TBase(init, bucket_count, hash, equal, alloc) {}

        THashMap(const THashMap& other) : TBase(other) {}
        THashMap(THashMap&& other) noexcept(std::is_nothrow_move_constructible_v<TBase>)
            : TBase(std::move(other)) {}
        auto operator=(const THashMap& other) -> THashMap& {
            TBase::operator=(other);
            return *this;
        }
        auto operator=(THashMap&& other) noexcept(std::is_nothrow_move_assignable_v<TBase>)
            -> THashMap& {
