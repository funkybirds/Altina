...
namespace AltinaEngine::Core::Container {
    ...
    public:
        ...
        explicit THashSet(size_type bucket_count, const Hash& hash = Hash(),
        ...
        THashSet(std::initializer_list<value_type> init, size_type bucket_count = 0,
            const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual(),
            const Allocator& alloc = Allocator())
            : TBase(init, bucket_count, hash, equal, alloc) {}

        THashSet(const THashSet& other) : TBase(other) {}
        THashSet(THashSet&& other) noexcept(std::is_nothrow_move_constructible_v<TBase>)
            : TBase(std::move(other)) {}
        auto operator=(const THashSet& other) -> THashSet& {
            TBase::operator=(other);
            return *this;
        }
        auto operator=(THashSet&& other) noexcept(std::is_nothrow_move_assignable_v<TBase>)
            -> THashSet& {
