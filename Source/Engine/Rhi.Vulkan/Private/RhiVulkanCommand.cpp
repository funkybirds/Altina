...
namespace AltinaEngine::Rhi {
    ...
    namespace {
        ...
        [[nodiscard]] auto HashAttachments(
            const TVector<VkFormat>& colors, VkFormat depthFormat, ERhiPrimitiveTopology topology)
            -> u64 {
        ...
        [[nodiscard]] auto BuildRenderPass(
            VkDevice device, const FRhiRenderPassDesc& desc, VkRenderPass& outRenderPass,
            VkFramebuffer& outFramebuffer, VkExtent2D& outExtent, TVector<VkClearValue>& clears,
            TVector<VkImageView>& attachments) -> bool {
            if (!device) {
                return false;
            }
            ...
            for (u32 i = 0; i < colorCount; ++i) {
                ...
                VkFormat format =
                    Vulkan::Detail::ToVkFormat(texture->GetDesc().mFormat);
            ...
            if (desc.mDepthStencilAttachment && desc.mDepthStencilAttachment->mView) {
                ...
                if (depthView) {
                    ...
                    VkFormat depthFormat =
                        Vulkan::Detail::ToVkFormat(texture ? texture->GetDesc().mFormat
                                                           : ERhiFormat::D32Float);
                    ...
                    attachment.stencilLoadOp  = ToVkLoadOp(desc.mDepthStencilAttachment->mStencilLoadOp);
                    attachment.stencilStoreOp = ToVkStoreOp(desc.mDepthStencilAttachment->mStencilStoreOp);
                    ...
                    clear.depthStencil.stencil = desc.mDepthStencilAttachment->mClearDepthStencil.mStencil;
                    clears.PushBack(clear);
    ...
    FRhiVulkanCommandPool::FRhiVulkanCommandPool(const FRhiCommandPoolDesc& desc, VkDevice device,
        u32 queueFamilyIndex, bool transient)
    ...
    void FRhiVulkanCommandContext::RHISetGraphicsPipeline(FRhiPipeline* pipeline) {
        ...
        if (mState->mGraphicsPipeline) {
            const VkPipeline pipelineHandle =
                mState->mGraphicsPipeline->GetOrCreatePipeline(
    ...
    void FRhiVulkanCommandContext::RHISetPrimitiveTopology(ERhiPrimitiveTopology topology) {
        ...
        if (mState->mSupportsExtendedDynamicState) {
            vkCmdSetPrimitiveTopology(mState->mCmd,
                Vulkan::Detail::ToVkPrimitiveTopology(topology));
        }
#else
        (void)topology;
#endif
    ...
    void FRhiVulkanCommandContext::RHISetIndexBuffer(const FRhiIndexBufferView& view) {
        ...
        if (buffer != VK_NULL_HANDLE) {
            vkCmdBindIndexBuffer(mState->mCmd, buffer,
                static_cast<VkDeviceSize>(view.mOffsetBytes), ToVkIndexType(view.mIndexType));
        }
#else
        (void)view;
#endif
    ...
    void FRhiVulkanCommandContext::RHISetRenderTargets(
        ...
        for (u32 i = 0; i < colorTargetCount; ++i) {
            ...
            auto rtvRef = static_cast<FRhiVulkanDevice*>(RHIGetDevice())
                ->CreateRenderTargetView(rtvDesc);
            if (rtvRef) {
        ...
        if (depthTarget) {
            ...
            depthView = static_cast<FRhiVulkanDevice*>(RHIGetDevice())
                ->CreateDepthStencilView(dsvDesc);
            if (depthView) {
    ...
    void FRhiVulkanCommandContext::RHIBeginRenderPass(const FRhiRenderPassDesc& desc) {
        ...
        if (mState->mSupportsDynamicRendering) {
            ...
            mState->mRenderingInfo.pDepthAttachment = hasDepth ? &mState->mDepthAttachment : nullptr;

            mState->mPipelineRenderingInfo = {};
            mState->mPipelineRenderingInfo.sType =
                VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;
            mState->mPipelineRenderingInfo.colorAttachmentCount =
                static_cast<u32>(mState->mColorFormats.Size());
            mState->mPipelineRenderingInfo.pColorAttachmentFormats =
                mState->mColorFormats.Data();
