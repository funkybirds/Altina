...
namespace AltinaEngine::Rendering::PostProcess::Builtin {
    ...
    void AddTonemap(RenderCore::FFrameGraph& graph, const RenderCore::View::FViewData& view,
        ...
        // HDR -> LDR (format like backbuffer, but still graph-managed).
        RenderCore::FFrameGraphTextureDesc outDesc{};
        outDesc.mDesc.mDebugName.Assign(TEXT("PostProcess.SceneColorLDR"));
        ...
        const f32 exposure = GetParamF32(node.Params, TEXT("Exposure"), 1.0f);
        const f32 gamma    = GetParamF32(node.Params, TEXT("Gamma"), 2.2f);
        ...
        desc.mName  = "PostProcess.Tonemap";
        ...
        graph.AddPass<FPassData>(
            ...
            [&](RenderCore::FFrameGraphPassBuilder& builder, FPassData& data) {
                ...
                rtvDesc.mDebugName.Assign(TEXT("PostProcess.SceneColorLDR.RTV"));
            ...
            [viewRect = view.ViewRect](Rhi::FRhiCmdContext& cmd,
                ...
                if (!Detail::BuildCommonBindGroupDesc(shared, Detail::kNameTonemapConstants,
                        shared.TonemapConstantsBuffer.Get(),
                        static_cast<u64>(sizeof(FTonemapConstants)), inTex, groupDesc)) {
                    return;
                }
                ...
                cmd.RHISetGraphicsPipeline(shared.TonemapPipeline.Get());
